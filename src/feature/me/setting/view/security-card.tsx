...
export function SecurityCard() {
  ...
  const { setupData, isSettingUp, hasSetup, handleSetup, handleCopySecret } =
    useSetupTotp()
  const { otpCode, setOtpCode, isOtpReady, isVerifying, handleVerify } =
    useConfirmTotp(setupData)
  const [isAuthenticatorEnabled, setIsAuthenticatorEnabled] = useState(false)
  const [showManualKey, setShowManualKey] = useState(false)
  const friendlyName =
    user?.full_name?.trim() || user?.email || "Personal authenticator"
  const handleAuthenticatorToggle = (checked: boolean) => {
    setIsAuthenticatorEnabled(checked)
    setShowManualKey(false)
    if (checked && !hasSetup) {
      void handleSetup(friendlyName)
    }
  }
      ...
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription>
              Add extra protection to keep your account secure.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
          ...
          <div className="space-y-6">
                    ...
                    <InputGroup>
                      ...
                      <InputGroupAddon align="inline-end">
                        <InputGroupButton
                          ...
                          disabled={!setupData?.secret || isSettingUp}
                        >
            ...
            <div className="grid gap-2">
              <Input
                ...
                disabled={!setupData?.mfa_id || isVerifying}
              />
            </div>
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <Button
                ...
                disabled={!setupData?.mfa_id || !isOtpReady || isVerifying}
              >
                Verify
              </Button>
            </div>
